...
from data_processing.etl import DATABASE_FILE
...
async def filter_pokemons(
    ...
    try:
        # Query to get name and HP only
        query = """
        SELECT DISTINCT 
            p.name,
            s_hp.base_stat as hp
        FROM pokemon p
        ...
        # Return list of objects with name and hp only
        return [
            {
                "name": row["name"],
